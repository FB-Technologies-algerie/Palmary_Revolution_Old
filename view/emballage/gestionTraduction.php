  ...
  <?php require('view/navbar.php') ?>
    ...
    <div class="card">
        <div class="card-header"  style="background-color:#D3D3D3; font-size:15px;border-top-left-radius: 15px;border-top-right-radius: 15px;">
           <span class="cardTitle" >DICTIONNAIRE DE TRADUCTIONS</span>         
        ...
        <div class="card-body">
            <div class="table-responsive">
                ...
                <?php while ($traduction=$listTraduction->fetch()) { ?>
                    <tr>
                      <td><a href="#" id="<?= $traduction['id_terme'] ?>" class="ldp_table" onclick="modifTranslat(this)"><?= $traduction['termeFR'] ?></a></td>
                      <td style="direction: rtl;" class="text-right"><?= $traduction['termeAR'] ?></td>
                      <td><?= $traduction['termeEN'] ?></td>
                      <td><?= $traduction['termePT'] ?></td>
                      <td><?= $traduction['termeES'] ?></td>
                      <td class="text-center">
                      <a><i class="fas fa-minus-circle" data-toggle="modal" data-target="#supprime" onclick="supprime(<?= $traduction['id_terme'] ?>)"></i></a>
                    </tr>
                <?php } ?>
                    </tbody>
                </table>
          ...
          <div class="modal-body">
           <div class="table-responsive">
             <table id="table" class="table table-striped table-hover table-bordered table-sm" >
              ...
              <tbody>
                     <tr>
                      <input type="hidden" name="id_terme" id="hiddenInput" value="">
    ...
    <script type="text/javascript">
      function supprimeTraduction(id_terme) {
        var xhttp;
          ...
          xhttp.open("GET", "<?= $_SESSION['url'] ?>gestionTraduction/supprimer/"+id_terme, true);
          xhttp.send(); 
        refresh();
      }
      ...
      console.log(translat[0].textContent);
      ...
      document.getElementById('portuguese').value = translat[3].textContent;
      document.getElementById('spanish').value = translat[4].textContent;
